/*
 * ADS1256_test.c:
 *	Nimmt Analog-Werte auf und stellt diese dar
 *
 */
...
#define  RST  RPI_GPIO_P1_18     //P1  18 statt 12, passend für unsere Platine RN
...
#define	SPICS2	RPI_GPIO_P1_16	//P4

#define CS_3() bcm2835_gpio_write(SPICS2,HIGH)
#define CS_2()  bcm2835_gpio_write(SPICS2,LOW)
...
*	name: getTemperature
*	function:  
*	parameter: Spannung 2 und 3: int32_t U2, int32_t U3
*	The return value:  float temperature
*********************************************************************************************************
*/

float getTemperature(int32_t U2, int32_t U3) {
	float R = 1000;
	float a = 1022;
	float b = 3.77;
	return U2 / (float)(U3) / b * R - a / b;
}
	
/*
*********************************************************************************************************
*	name: getTemperatureDelta
*	function:  
*	parameter: Spannung 2 und 3: int32_t U2, int32_t U3
*	The return value:  float d_temperature
*********************************************************************************************************
*/

float getTemperatureDelta(int32_t U2, int32_t U3) {
	/*float R = 1000;
	float a = 1022;
	double b = 3.77;
	float d_a = 2.;
	...
	double test = sqrt(b);
	//return sqrt(9);
	double de = 4.6;//(double)((d_b * (U2 * R / U3 / b / b - a / b / b)) * (d_b * (U2 * R / U3 / b / b - a / b / b)) + (d_a / b) * (d_a / b));
	return sqrt(4);
	double x = 1234321;
	double result;
	result = sqrt(x);
...
uint8_t start()
{
	...
	/*else
	{
		printf("Okkkkk, ADS1256 Chip ID = 0x%d\r\n", (int)id);
	}
	*/
	ADS1256_CfgADC(ADS1256_GAIN_1, ADS1256_15SPS);
    ADS1256_StartScan(1);
	
	/* Hardware-Initialisierung Ende */
	while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));
	while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));
	while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));while((ADS1256_Scan() == 0));
	
	t_anfang_0 = time_rechnung();
...
void ende()
{
    Write_DAC8532(0x30, Voltage_Convert(5.0,0.0));
    bcm2835_spi_end();
    bcm2835_close();
    printf("\nProgramm zu Ende");
}

int get_current(kanal)
{	if (kanal<4)
	{
...
float get_volt(int kanal)
{
	while((ADS1256_Scan() == 0));
	       
	for (eikl = 0; eikl < 8; eikl++)
	{
		adc[eikl] = ADS1256_GetAdc(eikl);
		volt[eikl] = (adc[eikl])*48/167;		// Umrechnungsfaktor (krumm)			 
	}
	if (volt[1] <= 0)
		{
			printf("\n Spannung 1: %i ", volt[1]);
	...
	return (float)volt[kanal]/1000000;
}

int time_rechnung(void) 
{
	struct timeval tv;
...
int time_ms()
{
	return time_rechnung() - t_anfang_0;
}

void wait_ms(int input_t)
{
	bsp_DelayUS(input_t*1000);
}
...
/*
float temp_sensor2(int adc)
{
	return (adc)*48/167;
}*/

void set_lamp(float power)
{
	Write_DAC8532(0x30, Voltage_Convert(5.0,power));
}
